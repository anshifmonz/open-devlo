...
function Elements({textInput, textAButton, FormCode}) {
  return (
    <div className="form-element">
      ...
      <p className="fw-5 mgb-1 lh-2 mgt-4 mgl-4"><b><InlineSyntaxHighlight lang={'html'} code={'<input type="text" name="username" placeholder="Enter your username">'} /></b>: Creates a single-line text input field.</p>
        ...
        <p className="fw-5 mgb-1 lh-2 mgt-6">The type attribute allow us to determine the type of input. The common ones are:</p>
        {
          textInput.map((item, i) => (
            <Fragment key={i}>
              ...
              <p className="fw-5 mgb-1 lh-2"><strong><InlineSyntaxHighlight lang={'html'} code={item.code} classes={"mgl-6"} /></strong>: {item.description}</p>
              {
                item.additionalCode &&
                <>
                  <p className="fw-5 mgb-1 lh-2"><strong><InlineSyntaxHighlight lang={'html'} code={item.additionalCode} classes={"mgl-6"}/></strong>: {item.additionalDescription}</p>
        ...
        textAButton.map((item, i) => (
          <Fragment key={i}>
            ...
            <div className="mgt-4 mgl-4">
              <p className="fw-5 mgb-1 lh-2"><strong><InlineSyntaxHighlight lang={'html'} code={item.tag} /></strong>: {item.desc}</p>
